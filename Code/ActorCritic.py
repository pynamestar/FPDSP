...
class ActorCritic:
    ...
    def update(self,transition_dict):
        ...
        #时序差分目标
        valelkfl=self.critic(next_states0,next_states1,next_states2)
        td_target=rewards+self.gamma*self.critic(next_states0,next_states1,next_states2)
        valsd= self.critic(states0,states1,states2)
        td_delta=td_target-self.critic(states0,states1,states2)
        # probs = self.actor(states0, states1, states2)
        # # probs=F.softmax(probs, dim=0)
        # mask = torch.tensor(env.mask, dtype=torch.float).to(self.device)
        # probs = F.softmax(probs + mask.log(), dim=0)  # 概率P
        AAA= self.actor(states0,states1,states2)
        BBB=AAA.gather(1,actions)
        log_probs = torch.log(self.actor(states0,states1,states2).gather(1,actions))
        actor_loss = torch.mean(-log_probs*td_delta.detach())  # PPO损失函数
