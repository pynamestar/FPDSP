...
class ActorCritic:
    ...
    def take_action(self,state):
        ...
        # print(state2)
        prob = self.q_net(state0, state1, state2)
    ...
    def update(self,transition_dict):
        ...
        q_values = self.q_net(states0,states1,states2).gather(1,actions)  # Q值
        # 下个状态的最大Q值
        max_next_q_values =self.target_q_net(next_states0,next_states1,next_states2).max(1)[0].view(-1, 1)
...
return_list,dqn_losses,totime_list,waittime_list,nodes_list,distance_list=trainer.train_off_policy_agent(env,agent,num_episodes,replay_buffer, minimal_size, batch_size)
