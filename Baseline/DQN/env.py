...
class Vrp_Env():
    ...
    def reset(self):
        # 随机选择一个i值，范围从1到80
        choicei = random.randint(1, 50)#choicei = random.randint(1, 80)
        self.node_sets = pd.read_csv("./datasets/node" + str(choicei) + ".csv").values
        self.ticoor_sets = pd.read_csv("./datasets/ticoor" + str(choicei) + ".csv").values
        self.order_sets = pd.read_csv("./datasets/order" + str(choicei) + ".csv").values
        # 当前订单是否可以访问,初始化为1都可以访问
        self.mask = np.repeat(1, self.order_sets.shape[0])
        # 车辆到每个订单当前目的地的距离,初始化为1
        # self.dismu = np.ones((2, self.order_sets.shape[0]))
