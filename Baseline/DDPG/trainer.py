...
import pandas as pd
writer = SummaryWriter('/root/tf-logs/DDPG30dim256d2')
def train_off_policy_agent(env, agent, num_episodes,replay_buffer, minimal_size, batch_size):
    ...
    for i in range(10):
        with tqdm(total=int(num_episodes/10), desc='Iteration %d' % i,ncols=100) as pbar:
            for i_episode in range(int(num_episodes/10)):
                ...
                # dqn_losses=agent.update(transition_dict)
                # actor_losses.append(actor_loss.detach().cpu().numpy())
                # critic_losses.append(critic_loss.detach().cpu().numpy())
    ...
    return return_list,actor_losses,critic_losses,totime_list,waittime_list,nodes_list,distance_list
