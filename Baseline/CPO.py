...
class Vrp_Env():
    # 读取节点文件(节点id及其相应位置id)
    node_sets = pd.read_csv("node.csv", dtype={'id': int, 'idt1': int, 'idt2': int}).values

    # 读取时间坐标文件(每个节点开始、结束时间和x、y坐标)
    ticoor_sets = pd.read_csv("ticoor.csv", dtype={'id': int, 'start_time': float, 'end_time': float, 'coor_x': float,
                                                   'coor_y': float}).values

    # 读取订单文件(送货节点、收货节点、货物量)
    order_sets = pd.read_csv("order.csv", dtype={'order_id': int, 'from_node_id': int, 'to_node_id': int,
...
if __name__ == "__main__":
    ...
    max_iterations = 4  # 最大迭代次数，与 ACO 代码一致
