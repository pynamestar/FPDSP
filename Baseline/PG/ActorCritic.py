...
#定义算法
class ActorCritic:
    def __init__(self,status_dim,action_dim,actor_lr,critic_lr,gamma,device):
        #策略网络
        self.policy_net=PolicyNet(action_dim).to(device)
        # 策略网络优化器
        self.optimizer=torch.optim.Adam(self.policy_net.parameters(),lr=actor_lr)
        self.gamma=gamma
        self.device=device
    def take_action(self,state):
        ...
        # print(state2)
        probs=self.policy_net(state0,state1,state2)
    ...
    def update(self,transition_dict):
        ...
        for i in reversed(range(len(rewards))):  # 从最后一步算起
            reward = rewards[i]
            states01 = torch.tensor([states0[i]], dtype=torch.float).to(self.device)
            states11 = torch.tensor([states1[i]], dtype=torch.float).to(self.device)
            states21 = torch.tensor([states2[i]], dtype=torch.float).to(self.device)
            action = torch.tensor([actions[i]]).view(-1, 1).to(self.device)
            log_prob = torch.log(self.policy_net(states01, states11, states21).gather(1, action))
    ...
    # 模型保存
    def save(self):
        torch.save(self.policy_net.state_dict(), 'models/net.pdparams')

    # 模型加载，只有权重
    def load(self):
        layer_state_dict = torch.load("models/net.pdparams")
        self.policy_net.load_state_dict(layer_state_dict)
...
agent=ActorCritic(status_dim,action_dim,actor_lr,critic_lr,gamma,device)
